...
def main():
    ...
    final = history[-1]
    print(f"Initial blue: {(history[0] == 1).mean():.1%}")
    print(f"Final blue:   {(final == 1).mean():.1%}")
    print(f"Final red:    {(final == -1).mean():.1%}")
    print(f"Iterations:   {len(history) - 1}")
    visualize(graph, history, "sim_pics/simulation.png")
    plot_timeline(history, "sim_pics/timeline.png")
    animate(graph, history, "sim_pics/simulation.gif")
